...
function Admin1Dashboard() {
  ...
  //       "http://localhost:3000/auth/send-email"
  //     );
  //     if (response.data.Status) {
  ...
  const approveleave = (EmpLeaveID) => {
    axios
      .post(`http://localhost:3000/auth/approveleave2/${EmpLeaveID}`)
      .then((response) => {
        if (response.data.Status) {
  ...
  const disapproveleave = (EmpLeaveID) => {
    axios
      .post(`http://localhost:3000/auth/disapproveleave2/${EmpLeaveID}`)
      .then((response) => {
        if (response.data.Status) {
          const updatedEmployeeData = employeeData.map((employee) =>
            employee.EmpLeaveID === EmpLeaveID
  ...
  const columns = [
    ...
    { title: "Start Date", dataIndex: "start_date", key: "start_date", width: "190px", render: formatDate },
    { title: "End Date", dataIndex: "end_date", key: "end_date", width: "120px", render: formatDate },
    {
      title: "Actions",
